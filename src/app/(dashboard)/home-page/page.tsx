...
export default function HomePageEditor() {
  ...
  const page = useQuery({
    queryKey: ["home-page"],
    queryFn: () => apiGet<HomePageContent>("/admin/home-page"),
  });

  useEffect(() => {
    if (page.data) setForm(page.data);
  }, [page.data]);
  const save = useMutation({
    mutationFn: () => apiPatch("/admin/home-page", form),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["home-page"] });
      show.success("Home page saved.");
    },
    onError: (reason) =>
      setError(reason instanceof Error ? reason.message : "Save failed."),
  });
  const update = <Key extends keyof HomePageContent>(
    key: Key,
  ...
  return (
    <div>
      <PageHeader
        title="Home page"
        description="Edit the main hero content and its call-to-action links."
      />
      <form
        className="w-full space-y-6 rounded-lg border border-border p-6"
        onSubmit={(event) => {
      ...
      >
        ...
        {form.pageMode === "countdown" && (
          <Field label="Countdown launch date">
            <Input
              ...
              onChange={(event) =>
                update("countdownTargetDate", event.target.value || undefined)
              }
              required
              type="date"
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Gold heading text">
            <Input
              value={form.titleAccent}
              onChange={(event) => update("titleAccent", event.target.value)}
            />
          </Field>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Gold CTA link">
            <Input
              value={form.secondaryCtaHref}
              onChange={(event) =>
                update("secondaryCtaHref", event.target.value)
              }
            />
          </Field>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Customers statistic">
            <Input
              value={form.customersValue}
              onChange={(event) => update("customersValue", event.target.value)}
            />
          </Field>
          <Field label="Satisfaction statistic">
            <Input
              value={form.satisfactionValue}
              onChange={(event) =>
                update("satisfactionValue", event.target.value)
              }
            />
          </Field>
        </div>
        <Button disabled={save.isPending} type="submit">
          {save.isPending ? <Spinner /> : null} Save home page
        </Button>
      </form>
